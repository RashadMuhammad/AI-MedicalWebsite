...
export default function ProfilePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    emergency_contact: user?.emergency_contact ? String(user.emergency_contact) : "",
  });
        ...
        <div className="flex items-center space-x-4">
          <div className="relative w-20 h-20 group">
            <Avatar className="w-20 h-20">
              {avatarPreview ? <AvatarImage src={avatarPreview} /> : <AvatarFallback>{user?.name?.[0]}</AvatarFallback>}
            </Avatar>
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground capitalize">{user?.role_name}</p>
          </div>
        </div>
